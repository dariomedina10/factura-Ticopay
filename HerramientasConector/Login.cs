...
namespace HerramientasConector
{
    public partial class Login : Form
    {
        ...
        {   
            ...
            {
                this.Text = "Herramientas para Conector Ticopay " + _tenant;
            }
        }
        ...
        private void LoginButton_Click(object sender, EventArgs e)
        {
            ...
            this._token = ExtenderToken(this._token);
        ...
        public string ExtenderToken(string token)
        {

            TicoPayDll.Response.Response respuestaServicio;
            RefreshCredentials credenciales = new RefreshCredentials();
            credenciales.Token = token;
            credenciales.AdditionalTimeType = TimeLapsType.Hours;
            credenciales.AdditionalTimeAmount = 1;
            respuestaServicio = Authentication.RefreshTokenAuthenticate(credenciales).GetAwaiter().GetResult();
            if (respuestaServicio.status == ResponseType.Ok)
            {
                JsonRefreshToken tokenFresco = JsonConvert.DeserializeObject<JsonRefreshToken>(respuestaServicio.result);
                return tokenFresco.objectResponse.tokenAuthenticate;
            }
            else
            {
